#ifndef CPP_FILTERS_FOOTGILDEDCONTROLLER_H_
#define CPP_FILTERS_FOOTGILDEDCONTROLLER_H_
#include <iostream>
#include <Eigen/Eigen>

namespace cpp_controllers {
  template <typename T>
  ...
  */
  ...
  template <typename T> T calcFootGuidedControl(const double& w, const T& l, const T& x0, const std::vector<LinearTrajectory<T> >& ur_) {
    ...
    if((1 - exp(-2 * w * Tj)) == 0.0) { // ゼロ除算チェック
      std::cerr << __FUNCTION__ << " (1 - exp(-2 * w * Tj))==0 !" << std::endl;
  ...
  template <typename T> void updateFootGuidedState(const double& w, const T& l, const T& c, const T& dc, const T& u, double m, double dt,
